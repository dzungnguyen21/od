...
function displayResultsBoard(detections, highestConfidenceDetection) {
  ...
  sortedDetections.forEach((detection) => {
    ...
    if (
      ...
      Math.abs(detection.confidence - highestConfidenceDetection.confidence) <
        0.001
    ) {
    ...
    item.innerHTML = `
            ...
            <div>
                ...
                ${
                  item.classList.contains("highest-confidence")
                    ? '<span class="highest-confidence-badge">Highest</span>'
                    : ""
                }
            </div>
        `;
    ...
    item.onclick = () => {
      if (preview.complete) {
        drawSingleDetection(preview, detection);
      }
      for (const child of detectionList.children) {
...
function drawDetectionsOnVideo(detections) {
  ...
  if (detections.length > 0) {
    ...
    resultText.textContent =
      "Real-time detection active... No objects detected";
  }
}
  ...
  if (navigator.mediaDevices && navigator.mediaDevices.getUserMedia) {
    navigator.mediaDevices
      ...
      .then((stream) => {
        ...
        preview.style.display = "none";
        resultText.textContent =
          'Camera ready. Click "Start Real-time Detection" or "Capture Photo".';
...
function handleUploadFormSubmit(e) {
  ...
  if (capturedBlob && !finalImageFile) {
    finalImageFile = new File([capturedBlob], "capture.jpg", {
      type: "image/jpeg",
    });
  }
    ...
    .then((data) => {
      ...
      if (data.detections && data.detections.length > 0) {
        ...
        if (data.highest_confidence_detection) {
          ...
          const highConfidenceCount = data.detections.filter(
            (d) => d.confidence > 0.8
          ).length;
          const isShowingTop10 = highConfidenceCount === 0;
          const statusText = isShowingTop10
            ? " (from top 10 detections)"
            : " (confidence > 80%)";

          resultText.textContent = `Detection complete! Showing bounding box for highest confidence object: ${
            data.highest_confidence_detection.label
          } (${(data.highest_confidence_detection.confidence * 100).toFixed(
            1
          )}%)${statusText}`;
        } else {
        ...
        resultsBoard.style.display = "none";
        resultText.textContent =
          "Server processed the image, but no objects were found.";
      }
    })
    .catch((err) => {
